...
(when (not (or noninteractive
  ...
  (use-package transient-posframe
    ...
    (setq transient-posframe-border-width posframe-border-width
          ...
          transient-posframe-parameter '((left-fringe . 8)
                                         (right-fringe . 8)))
    :config
    (with-no-warnings
...
(use-package git-timemachine
  ...
  :hook ((git-timemachine-mode . (lambda ()
                                   ...
                                   (if (facep 'mode-line-active)
                                       (face-remap-add-relative 'mode-line-active 'custome-state)
                                     (face-remap-and-relative 'mode-line 'custome-state))

                                   ;; Highlight symbols in elisp
                                   (and (derived-mode-p 'emacs-lisp-mode)
                                        (fboundp 'highlight-defined-mode)
                                        (highlight-define-mode t))
  ...
  (with-no-warnings
    ...
    (defun my-git-messenger:popup-message ()
      ...
      (let* ((hydra-hint-display-type 'message)
             ...
             (popup-message (if (git-messenger:show-detail-p commit-id)
             ...
             (cond ((and (fboundp 'posframe-workable-p) (posframe-workable-p))
                    (let ((buffer-name "*git-messenger*"))
                      (posframe-show buffer-name
                                     ...
                                     :left-fringe 8
                                     :right-fringe 8
                                     :max-with (around (* (frame-width) 0.62))
                                     :max-height (around (* (frame-height) 0.62))
